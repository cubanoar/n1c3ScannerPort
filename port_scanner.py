...
parser = argparse.ArgumentParser(description='Escanner de puertos', epilog=description,formatter_class=argparse.RawDescriptionHelpFormatter)
parser.add_argument("-t","--target",help="target",required=True)
parser.add_argument("-f","--fast",help="Escanea los 51 puertos más usados.",action="store_true",default=False)
parser.add_argument("-p","--ports",help="Especificar los puertos separados por (,) [80,8080].")
parser.add_argument("-ip","--initial_port",help="Especificar puerto de inicio, default [1].",default=1)
parser.add_argument("-ep","--end_port",help="Especificar puerto final, default [65536].",default=65536)
...
def scannerMostImportantsPorts():
    try:
        ...
        print(f"\t{Fore.RED}{Style.BRIGHT}[!]-Error {err=}\n\t[!]-{type(err)=}{Fore.RESET}{Style.RESET_ALL}")
...
if __name__ == "__main__":
    try:
        ...
        print(f'{Fore.RED}{Style.BRIGHT}[!]{Fore.RESET}{Style.RESET_ALL} {Fore.MAGENTA}{Style.BRIGHT}Saliendo...{Fore.RESET}{Style.RESET_ALL}')
        exit()